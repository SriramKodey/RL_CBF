...
def main(env, policy):
    ...
    cbf_b_values = get_cbf_vals(states, controls)
    return observations[:, 0], cbf_b_values
...
if __name__== "__main__":
    ...
    x_pos_1, cbf_b_values_1 = main(env, actor_1)

    path_2 = "ddpgcbf_actor_model.pt"
    actor_2 = load_policy(path_2, state_dim, action_dim, max_action)
    x_pos_2, cbf_b_values_2 = main(env, actor_2)

    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 10))
    # Plot x_pos_1 and x_pos_2 on the first subplot
    ax1.plot(x_pos_1, color='lime', label='DDPG', linewidth=5)
    ax1.plot(x_pos_2, color='red', label='DDPG-CBF', linewidth=5)
    ax1.axhline(0.7, color='k', linestyle='--', label='Bounds', linewidth=4)
    ax1.axhline(-0.7, color='k', linestyle='--', linewidth=5)
    ax1.set_xlabel('Time Steps', fontsize=24)
    ax1.set_ylabel('Cart Position (m)', fontsize=24)
    ax1.legend(fontsize=16, loc='upper right')

    # Plot cbf_b_values_1 and cbf_b_values_2 on the second subplot
    ax2.plot(cbf_b_values_1, color='lime', label='DDPG', linewidth=5)
    ax2.plot(cbf_b_values_2, color='red', label='DDP-CBF', linewidth=5)
    ax2.axhline(0, color='k', linestyle='--', label='Threshold', linewidth=4)
    ax2.set_xlabel('Time Steps', fontsize=24)
    ax2.set_ylabel('$b(z)$ Values', fontsize=24)
